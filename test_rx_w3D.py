...
# ============ VISUAL TWEAKS ============
MODEL_OFF_WHITE = (0.98, 0.99, 0.99, 1.0)   # >>> TWEAK HERE: off-white body color (RGBA 0..1)
MODEL_EDGE_COLOR = (0.50, 0.50, 0.50, 1.0)  # edges slightly darker
FIN_OFF_WHITE = (0.90, 0.90, 0.90, 1.0)     # fins a touch darker than body

# Raise rotation pivot by a fraction of model height (positive raises the pivot towards the nose)
PIVOT_RAISE_FRAC = 0.12                      # >>> TWEAK HERE: 0.0 = center, 0.12 ≈ “a bit higher”
# =======================================


def meshdata_from_stl(path: str, scale: float = 1.0) -> MeshData:
    """
    Load an STL and return pyqtgraph.opengl.MeshData.
    Tries trimesh first, then numpy-stl. Raises if neither available or load fails.
    """
    if _HAS_TRIMESH:
    ...
    if _HAS_NUMPY_STL:
        ...
        tris = np.asarray(m.vectors, dtype=np.float32) * float(scale)  # (n, 3, 3)
...
def _rotation_matrix_from_a_to_b(a: np.ndarray, b: np.ndarray) -> np.ndarray:
    """Rodrigues rotation: rotate vector a to vector b. Returns 3x3."""
...
class Rocket3DWidget(QtWidgets.QWidget):
    """
    3D rocket that rotates with roll/pitch (roll=X, pitch=Y, yaw=Z).
    Rocket points along +Z (nose up).
    """
    def __init__(self, parent=None):
        super().__init__(parent)

        TARGET_SIZE = 2.5      # auto-fit target
        self.SCALE = 1.0       # extra multiplier after auto-fit
        ...
        # --- Load STL (fallback to simple rocket if missing) ---
        md = None
        try:
        ...
        if md is not None:
            ...
            # Align longest axis to +Z (PCA)
            cov = np.cov(Vc.T)
            eigvals, eigvecs = np.linalg.eigh(cov)  # ascending
            principal_dir = eigvecs[:, -1]          # longest axis
            R = _rotation_matrix_from_a_to_b(principal_dir, np.array([0.0, 0.0, -1.0], dtype=np.float32))
            Vc = (Vc @ R.T).astype(np.float32)

            # Disambiguate "up": nose (narrow end) must be at +Z
            z = Vc[:, 2]
            zmin, zmax = float(z.min()), float(z.max())
            span = max(1e-6, (zmax - zmin))
            top_mask = z >= (zmin + 0.85 * span)      # top 15%
            bot_mask = z <= (zmin + 0.15 * span)      # bottom 15%

            def median_radius(mask):
                pts = Vc[mask]
                if len(pts) == 0:
                    return 0.0
                r = np.linalg.norm(pts[:, :2], axis=1)  # radius in XY
                return float(np.median(r)) if len(r) else 0.0

            r_top = median_radius(top_mask)
            r_bot = median_radius(bot_mask)

            # If top is wider than bottom, it's upside down -> rotate 180° about X
            if r_bot > r_top:
            ...
            # ===== Raise pivot a bit (moves mesh down so rotation point is higher) =====
            z = Vc[:, 2]
            zmin, zmax = float(z.min()), float(z.max())
            height = max(1e-6, (zmax - zmin))
            Vc[:, 2] -= float(PIVOT_RAISE_FRAC) * height     # >>> TWEAK HERE if needed
            # ==========================================================================
            ...
            # --- Fallback simple rocket (body + two fins), off-white ---
            verts = np.array([
                [ 0.0,   0.0,   1.5],   # nose
                [-0.12, -0.12, -1.5],
                [ 0.12, -0.12, -1.5],
            ...
            # Raise pivot for fallback mesh too
            zmin, zmax = float(verts[:,2].min()), float(verts[:,2].max())
            height = max(1e-6, (zmax - zmin))
            verts[:, 2] -= float(PIVOT_RAISE_FRAC) * height  # >>> TWEAK HERE if needed
            ...
            self._model_scale = 2.2  # nice size

        # Draw once with scale so it appears immediately
    ...
    def set_attitude(self, pitch_deg: float, roll_deg: float):
        ...
        roll  = float(roll_deg)
        self._apply_transform([
            lambda item: item.rotate(pitch, 0, 1, 0),  # pitch (Y)
            lambda item: item.rotate(roll,  1, 0, 0),  # roll  (X)
        ])
    ...
    def set_attitude_rpy(self, roll_deg: float, pitch_deg: float, yaw_deg: float):
        if not self._parts:
            return
        self._apply_transform([
            lambda item: item.rotate(roll_deg,  1, 0, 0),  # X
            lambda item: item.rotate(pitch_deg, 0, 1, 0),  # Y
            lambda item: item.rotate(yaw_deg,   0, 0, 1),  # Z
        ])
